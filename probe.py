...
def load_data(model_name, dynamic, belief, variable="belief"):
    N = 200
    test_num = 100
    feats_all = []
    labels_all = []
    ...
    for i in range(N):
        for setting_name in ['true', 'false']:
            for belief_statement in ['true', 'false']:
                feats = np.load('data/results/representations/%s/%s_%s_%s_belief/reps_0shot_%s_%s_belief_%s_%d_attention.npy' % (model_name, dynamic, variable, setting_name, variable, setting_name, belief_statement, i))
...
if __name__ == "__main__":
    ...
    train_acc_all, val_acc_all, roc_auc_all, logloss_all, coefs_all, CoMs_all \
        = probe_all(all_X, all_y, test_size=0.2, seed=args.seed)

    # Save the results.
    ensure_dir(f'data/results/probe/{args.model_name}')
    plot_heatmap(train_acc_all, "Probe Train Acc.", save_path="data/results/probe/%s/%s_%s_%s_train_acc" % (args.model_name, args.dynamic, args.variable, args.belief))
    plot_heatmap(val_acc_all, "Probe Val Acc.", save_path="data/results/probe/%s/%s_%s_%s_val_acc" % (args.model_name, args.dynamic, args.variable, args.belief))
    plot_heatmap(roc_auc_all, "ROC AUC Val", save_path="data/results/probe/%s/%s_%s_%s_val_auc" % (args.model_name, args.dynamic, args.variable, args.belief))

    np.save("data/results/probe/%s/%s_%s_%s_val_acc.npy" % (args.model_name, args.dynamic, args.variable, args.belief), val_acc_all)
    np.save("data/results/probe/%s/%s_%s_%s_coef.npy" % (args.model_name, args.dynamic, args.variable, args.belief), coefs_all)
